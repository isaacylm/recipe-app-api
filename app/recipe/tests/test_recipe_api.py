...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_create_recipe_with_new_ingredients(self):
        """Test creating recipe with new ingredients is succesful."""
        payload = {
            'title': 'Cappucino',
            'time_minutes': 5,
            'price': Decimal('9.80'),
            'ingredients': [{'name': 'Coffee'}, {'name': 'Milk'}],
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
    ...
    def test_create_ingredient_on_update_in_recipe(self):
        """Test updating an ingredient on a recipe is successful."""
        recipe = create_recipe(user=self.user)
        payload = {
    ...
    def test_filter_by_tags(self):
        ...
        recipe1 = create_recipe(user=self.user, title='Feijoada')
        recipe2 = create_recipe(user=self.user, title='Macarronada')
        tag1 = Tag.objects.create(user=self.user, name='Lunch')
        tag2 = Tag.objects.create(user=self.user, name='Heavy Carbohydrates')
        recipe1.tags.add(tag1)
        recipe2.tags.add(tag2)
        recipe3 = create_recipe(user=self.user, title='Moqueca')
